...
class ProtFlexFilterPCA(ProtAnalysis3D, ProtFlexBase):
    ...
    """
            The ProtFlexFilterPCA protocol performs filtering of conformational
            latent spaces using Principal Component Analysis (PCA). Its main
            purpose is to simplify flexibility embeddings by preserving only
            the most relevant principal components while removing less informative
            variability from the conformational landscape.

            Inputs and General Workflow

            The protocol requires a set of flexible particles containing latent
            flexibility descriptors generated by methods such as Zernike3D,
            HetSIREN, or CryoDRGN. Users define which PCA components should be
            preserved during filtering. Components can be specified individually,
            as ranges, or as a percentage of cumulative explained variance.

            During execution, the protocol extracts the latent flexibility
            coordinates associated with each particle and computes a PCA
            decomposition of the conformational space. The selected principal
            components are preserved while the remaining components are removed.
            The filtered latent space is then reconstructed through inverse PCA
            transformation, generating a simplified conformational representation.

            Outputs and Interpretation

            After execution, the protocol produces a new set of flexible particles
            containing the filtered latent coordinates. All original particle
            metadata and alignment information are preserved during the process.
            The protocol also stores the filtered PCA coordinates and the explained
            variance associated with the preserved components.

            The resulting conformational space contains only the selected modes
            of variability, allowing users to focus on the dominant structural
            motions present in the dataset while reducing noise and less relevant
            fluctuations.

            Biological Perspective

            PCA filtering is particularly useful in cryo-EM flexibility analysis
            because conformational landscapes often contain noisy or low-impact
            variability components that complicate interpretation. By preserving
            only the principal structural motions, this protocol facilitates the
            analysis of biologically meaningful conformational transitions and
            improves the interpretability of heterogeneous datasets.

            Final Perspective

            ProtFlexFilterPCA provides an efficient strategy for simplifying
            conformational latent spaces while preserving the dominant structural
            variability of cryo-EM datasets. This enables cleaner visualization,
            more robust downstream analysis, and improved interpretation of
