...
class ProtFlexDimRedSpace(ProtAnalysis3D, ProtFlexBase):
    ...
    """
            The ProtFlexDimRedSpace protocol performs dimensionality reduction on
            flexibility embeddings associated with cryo-EM particles. Its main purpose
            is to transform high-dimensional conformational spaces into reduced latent
            landscapes that are easier to visualize, interpret, and analyze while
            preserving the structural relationships between particles.

            Inputs and General Workflow

            The protocol requires a set of flexible particles containing previously
            computed flexibility descriptors generated by methods such as Zernike3D
            or CryoDrgn. During execution, the protocol extracts the latent coordinates
            associated with each particle and computes a reduced embedding using one
            of the available dimensionality reduction strategies.

            Several reduction methods are supported. UMAP generates nonlinear embeddings
            that usually preserve conformational relationships more effectively. PCA
            provides a faster linear reduction approach suitable for exploratory analyses.
            The deepElastic method uses a variational autoencoder architecture to learn
            compact embeddings while preserving the clustering structure of the original
            latent space.

            The protocol allows users to define the dimensionality of the reduced space
            and configure advanced parameters depending on the selected method. GPU
            acceleration can also be enabled for computationally demanding calculations
            such as UMAP and deepElastic training.

            Outputs and Interpretation

            After execution, the protocol produces a new set of flexible particles
            containing the reduced latent coordinates for each particle. All original
            metadata and alignment information are preserved. When UMAP is used, the
            trained embedding model is additionally stored for future reuse.

            The resulting reduced landscape can be used for visualization, clustering,
            trajectory analysis, and exploration of conformational variability within
            heterogeneous cryo-EM datasets.

            Biological Perspective

            Dimensionality reduction is an essential step in flexibility analysis because
            high-dimensional latent spaces are difficult to interpret directly. By
            generating compact embeddings, this protocol facilitates the identification
            of conformational continua, structural transitions, and discrete molecular
            states present in cryo-EM datasets.

            Final Perspective

            ProtFlexDimRedSpace provides an efficient framework for simplifying complex
            conformational landscapes while preserving biologically relevant structural
            relationships. This improves the interpretability of flexibility analyses
            and supports downstream exploration of structural heterogeneity in cryo-EM
            workflows.
        """
    _label = 'dimred space'
    _devStatus = NEW
